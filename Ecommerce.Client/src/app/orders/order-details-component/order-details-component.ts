...
export class OrderDetailsComponent implements OnInit {
  ...
  getOriginalPrice(price: number, discountPercentage?: number): number {
    if (!discountPercentage || discountPercentage <= 0) {
      return price;
    }
    // Price represents the discounted price already.
    // We reverse engineer original price from the discount.
